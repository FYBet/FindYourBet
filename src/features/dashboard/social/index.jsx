      ...
      <div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            {active.map(c => (
              <div key={c.id} onClick={() => handleOpenConv(c)}
                style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', background: 'var(--color-bg)', border: '0.5px solid var(--color-border)', borderRadius: 'var(--radius-lg)', cursor: 'pointer', transition: 'border-color 0.15s' }}
                onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--color-primary-border)'}
                onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--color-border)'}>
                <div style={{ position: 'relative' }}>
                  <div style={{ width: '44px', height: '44px', background: 'var(--color-primary-light)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '17px', fontWeight: 700, color: 'var(--color-primary)', flexShrink: 0 }}>
                    {(c.otherUsername || '?')[0].toUpperCase()}
                  </div>
                  {c.unread > 0 && (
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2px' }}>
                    <div style={{ fontWeight: c.unread > 0 ? 700 : 600, fontSize: '14px' }}>@{c.otherUsername}</div>
                  ...
                  <div style={{ fontSize: '13px', color: c.unread > 0 ? 'var(--color-text)' : 'var(--color-text-muted)', fontWeight: c.unread > 0 ? 600 : 400, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
